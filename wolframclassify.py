...
def generate_IP(num_population):
	"""Generates an initial population of 8-bit digits to be used for our representation"""
	population=[]
...
def step(bin_pop, initial_IC):
	...
	for i in range(len(initial_IC)):
		...
		if i==0:
			value=2*initial_IC[i]+1*initial_IC[i+1]
		elif i==len(initial_IC)-1:
			value=4*initial_IC[i-1]+2*initial_IC[i]
		else:
			# Full case, convert the neighborhood to a digit from 0 to 8
			value=4*initial_IC[i-1]+2*initial_IC[i]+initial_IC[i+1]
		# binary population is a string represented in binary
		#print("bin pop", bin_pop)
		#print("value", value)
		new_value=int(bin_pop[7-value])
...
def compute_results(IP, length, cutoff):
	...
	a1=generate_IC(25, length, 0.2)
	#a3=generate_IC(25, length, 0.3)
	#a3=generate_IC(10, length, 0.3)
	#a4=generate_IC(10, length, 0.4)
	#a5=generate_IC(10, length, 0.5)
	#a6=generate_IC(10, length, 0.6)
	#a7=generate_IC(25, length, 0.7)
	a8=generate_IC(10, length, 0.8)
	#a9=generate_IC(25, length, 0.9)
	...
	for population in IP:
		...
		counter=0
		for IC in IC_list:
			counter+=solve(IC, population, 2*length)
...
def evolve(IP, number, rate):
	...
	for _ in range(number):
		...
		bin_pop0='{:08b}'.format(p0)
		bin_pop1='{:08b}'.format(p1)
		...
		if cutoff>=1:
			...
			cutoff=0
		cutoff=int(round(8*cutoff))
		#do single-cutoff crosovver
		bin_pop=bin_pop0[0:cutoff]+bin_pop1[cutoff:8]
		other=bin_pop1[0:cutoff]+bin_pop0[cutoff:8]
		value=int(bin_pop, 2)
...
def main():
	IP=generate_IP(256)
	best=compute_results(IP, 39, 50)
main()
#compute_results([248], 23, 1)